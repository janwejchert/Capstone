...
\emph{Reflexivity} refers to situations in which beliefs and actions help shape the environment they are meant to describe. In financial markets this means that traders do not merely observe prices; through their orders they also contribute to price formation. The proposal uses the term in a precise and limited way: a forecast influences trading, trading influences returns, and returns then determine whether the forecast continues to perform well, against either the realised or the residual return target (Lo 2004; MacKenzie 2006).
...
Fourth, empirical finance provides a practical motivation. McLean and Pontiff (2016) document that return predictors tend to weaken outside their discovery sample and weaken further after publication. Khandani and Lo (2011) document the August 2007 quant crisis as an extreme realisation of strategy crowding. MacKenzie and Millo (2003) and MacKenzie (2006) provide the canonical case study of a financial model (Black-Scholes) reshaping the very market it described. That evidence does not prove a single mechanism, but it is consistent with the possibility that awareness and imitation reduce certain measures of predictability while leaving others intact. The present proposal does not claim to identify those empirical effects directly. Instead, it builds a clean market laboratory for one plausible mechanism behind them.

\subsection{Positioning and contribution}

The gap in the literature is not the absence of intuition. Economists, finance researchers, and machine-learning theorists already recognise that behaviour can change predictive environments. The unresolved issue is operational: there is still limited evidence from a transparent and disciplined market simulation showing how a shared forecasting rule can diffuse, alter market dynamics, and change its measured value separately along realised, residual, and economic targets.
...
\subsubsection{Baseline return equation and the realised vs residual decomposition}

The first-differenced version of (5) gives a one-period market-maker return of $\mu D_t + \sigma \varepsilon_{t+1}$. The baseline return law augments this market-maker return with a residual AR(1) component carried in from prior periods, giving
\begin{equation}
...
For evaluation purposes, it is useful to rewrite (6) as a decomposition into a contemporaneous price-impact component and a residual:
\begin{equation}
  r_{t+1} \;=\; \mu\, D_t \;+\; x_{t+1}, \qquad x_{t+1} \;=\; \varphi\, r_t \;+\; \sigma\, \varepsilon_{t+1}.
\end{equation}
The residual return $x_{t+1}$ contains the part of the return that does not depend on contemporaneous adopter trading: the lagged-return component and the exogenous news shock. The forecast $\hat{r}_{t+1}^{(A)}$ is, by construction, an estimate of the AR component of the return process and therefore an estimate of $x_{t+1}$ in expectation. The proposal evaluates the forecast against both $r_{t+1}$ and $x_{t+1}$ and reports the two readings as separate primary endpoints.
...
\begin{enumerate}[label=\arabic*., leftmargin=2em, itemsep=0.2em]
  ...
  \item The exogenous news shock $\sigma \varepsilon_{t+1}$ and the residual autoregressive term $\varphi r_t$ realise.
\end{enumerate}

Given this timing, the forecast can be evaluated against either of two distinct targets, and the proposal reports both as primary endpoints.

\textbf{Self-fulfilling channel.} The forecast is compared to the \emph{realised} return $r_{t+1} = \mu D_t + x_{t+1}$. When many adopters trade on a positive $\hat{r}_{t+1}^{(A)}$, the contemporaneous $\mu D_t$ component pushes $r_{t+1}$ in the same direction as the forecast. The realised-return reading therefore answers the question: does the forecast correctly point at where the realised price goes, including its own contemporaneous price impact?

\textbf{Residual-erosion channel.} The forecast is compared to the \emph{residual} return $x_{t+1} = r_{t+1} - \mu D_t$. The residual contains the lagged-return component plus exogenous news, that is, the part of the return process the forecast was actually designed to estimate. The residual-return reading therefore answers the question: after removing the contemporaneous demand impact, does the forecast still carry independent predictive content?

The two channels are not in conflict. They answer different questions about the same forecast on the same path, and they can move in opposite directions as adoption rises. Both are reported under their own names, on equal footing, and neither is treated as the canonical evaluation target.
...
\begin{itemize}[itemsep=0.2em]
  ...
  \item \textbf{Retraining frequency.} Compare slow versus frequent model updates to examine whether retraining delays, freezes, or accelerates the residual-erosion channel (Mendler-Dunner et al. 2020; Kabra and Patel 2024).
...
The metric set is organised into three primary endpoints, a diagnostic set, and a threshold-metric block. The realised-return and residual-return readings of the forecast are reported as separate primary endpoints, in line with the dual-channel framing of section 3.6.

\subsection{Primary endpoints}

\subsubsection{Realised-return predictive performance (self-fulfilling diagnostic)}
...
\end{equation*}
computed in a trailing window ending at $t$. This measures the self-fulfilling channel. It can rise with adoption because contemporaneous adopter demand pushes the realised return in the forecast direction.

\subsubsection{Residual-return predictive content (statistical erosion endpoint)}

The second primary metric is the out-of-sample $R^{2}$ of the forecast against the residual return,
\begin{equation*}
  R^{2}_{\mathrm{resid}}(t) \;=\; 1 \;-\; \frac{\sum (x_{s+1} - \hat{r}^{(A)}_{s+1})^{2}}{\sum (x_{s+1} - \bar{x})^{2}}, \qquad x_{s+1} = r_{s+1} - \mu D_{s}.
\end{equation*}
This measures the residual-erosion channel. It can fall with adoption because the rolling fit picks up the amplified empirical autocorrelation from realised returns and over-predicts the residual process. The two $R^{2}$ readings are reported on equal footing as separate primary endpoints; neither is treated as the canonical evaluation target.

\subsubsection{Effective AR coefficient (market-dynamics diagnostic)}

The rolling effective AR coefficient $\hat{\varphi}^{\mathrm{eff}}(t)$ is estimated directly from the realised return series in a trailing window. In the implemented return law (6), adopter buying on a positive AR forecast reinforces the lagged-return component through $\mu D_t$, so $\hat{\varphi}^{\mathrm{eff}}$ is expected to rise with adoption. A naive reading of the same return law might instead predict $\hat{\varphi}^{\mathrm{eff}}$ to fall, on the grounds that contemporaneous absorption through $\mu D_t$ should reduce the lagged coefficient carried into next-period returns; that reading does not survive once the sign of the price-impact term is tracked carefully. The market-dynamics diagnostic $\hat{\varphi}^{\mathrm{eff}}$ is reported alongside the two $R^{2}$ endpoints, because it is the mechanical reason $R^{2}_{\mathrm{resid}}$ falls while $R^{2}_{\mathrm{realised}}$ rises.
...
\subsection{Threshold metrics: endpoint-specific critical adoption shares}

A single combined critical adoption share across endpoints is ambiguous: the same forecast can erode along one endpoint while persisting along another. The proposal therefore reports one threshold per primary endpoint, so the result reads honestly under each reading of "the rule has stopped paying off".

\begin{itemize}[itemsep=0.2em]
  \item \textbf{$A^{*}_{R^{2},\mathrm{resid}}$}: smallest adoption share at which the rolling residual-return $R^{2}$ crosses zero. If the cost-free baseline produces no zero crossing, a pre-registered relative threshold is used (for example, $R^{2}_{\mathrm{resid}}$ falls to half its low-adoption baseline). The relative version is labelled $A^{*}_{R^{2},\mathrm{resid},\mathrm{rel}}$ to keep it distinct from the zero-crossing definition.
  \item \textbf{$A^{*}_{\mathrm{profit}}$}: smallest adoption share at which net profit relative to the null benchmark crosses zero, at a given transaction-cost level.
...
For the realised-return $R^{2}$ channel, the corresponding "first crossing" need not exist in the direction the proposal anticipates a priori: realised $R^{2}$ may rise with adoption, in which case there is no realised-return erosion threshold to report. This is itself a finding and is recorded as such; an empty $A^{*}_{R^{2},\mathrm{realised}}$ field carries information.

Reporting endpoint-specific $A^{*}$ across parameter regimes converts the qualitative dual-channel claim into a quantitative one, and lets the reader see which endpoints actually erode, at what adoption share, and under which $(\mu, \varphi, w)$ combinations.

\subsection{Expected results}

The dual-channel framing replaces a single direction for "forecast accuracy and profitability" with conditional expectations along the three primary endpoints. These expectations are guided by analytical inspection of the return law (6) and by the implemented simulator results.

\begin{enumerate}[itemsep=0.2em]
  \item \textbf{Realised-return $R^{2}$ may rise with adoption.} Self-fulfilment can dominate erosion when the price-impact channel is strong. There may be no realised-return erosion threshold.
  \item \textbf{Residual-return $R^{2}$ should decline with adoption} if the rolling fit picks up the amplified empirical autocorrelation and over-predicts the residual process. The implementation in phases 1 to 6 confirms this with tight Monte Carlo bands across paired-shock seeds.
  \item \textbf{Effective AR coefficient should rise with adoption} in the implemented return law (6), because adopter trading reinforces rather than offsets the lagged-return component.
  \item \textbf{Economic value may persist or rise in the cost-free baseline} because adopter trades coordinate and the realised return moves in the forecast direction. Whether the rule's economic edge survives once transaction costs are introduced depends on the cost level; phase 7 maps that dependence.
  \item \textbf{Endogenous performance-based switching} (CE-rule) and exogenous stochastic diffusion produce the same terminal residual erosion and effective-$\varphi$ amplification at full adoption; the path through adoption space differs but the steady state agrees, in the regimes tested.
\end{enumerate}

A particularly informative result would be a threshold pattern in $A^{*}_{R^{2},\mathrm{resid}}$: residual predictive content stable at low adoption, declining sharply once adoption exceeds the threshold. That would be strong evidence that crowding, rather than ordinary forecasting noise, is responsible for residual-channel erosion.

\subsection{Scope and limitations}

The proposal also admits meaningful null results. Residual-channel erosion may fail to appear if price impact is too weak, if adoption remains sparse, or if the predictive relation is robust enough to survive imitation. Such findings would still be scientifically useful: they would identify the conditions under which deployment-induced residual erosion is limited rather than pervasive. A well-specified null result is therefore not a failure of the design; it is evidence about the boundary of the mechanism.

One specific outcome that the dual-channel framing makes safe to report is the pattern where realised-return $R^{2}$ \emph{rises} while residual $R^{2}$ \emph{falls}. Both channels are reported under their own names, so a positive realised-return result and a negative residual-return result do not have to be reconciled into a single direction before the reader sees them.

At the same time, the conclusions should remain carefully bounded. Positive findings would support the proposition that residual-channel forecast erosion is a plausible and measurable mechanism in a class of single-asset markets. They would \emph{not} establish that all return predictors fail for performative reasons, that the same mechanism explains every empirical anomaly, or that autoregressive models are realistic descriptions of actual trading practice.
...
\midrule
\textbf{1. Baseline} &
Implement the single-asset market with return law (6), random null rule (8), Brock-Hommes demand (1), and reproducible simulation routines. &
...
Introduce stochastic adoption (eqs 11 to 12) and run paired-shock regimes. &
Adoption paths; realised and residual $R^{2}$ versus adoption share; effective $\varphi$ versus adoption share. \\
\midrule
\textbf{5. CE switching} &
Replace stochastic diffusion with the certainty-equivalent switching rule (eqs 13 to 15). &
Stochastic versus performance-based adoption comparison; same terminal residual erosion. \\
\midrule
\textbf{6. Experiments} &
Sweep $(\mu, \varphi, w, p)$; locate endpoint-specific critical adoption shares; report robustness across AR order. &
Heatmaps for $A^{*}_{R^{2},\mathrm{resid},\mathrm{rel}}$ and an effective-$\varphi$ threshold; paired-seed erosion curves. \\
\midrule
\textbf{7. Evaluation} &
